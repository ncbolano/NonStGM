...
if(length(args)==0){
  ...
  n <- 2^12
  #M_grid <- c(14,20, 30, 40, 50, 60)  # Grid for M selection
  TV_size=0.6
  L=1
...
filenames=sprintf('n_%i_nu_%i_L_%i_TV_%0.2f_%s_extractK_localM',n,nu,L,TV_size,Kernel)
################################################################
...
sim_Res=foreach(i=icount(R),.combine = cbind,.inorder = FALSE,.errorhandling = "stop") %dopar% {
  ...
  Ms <- numeric(n_k)  # Store adaptive M values for each k
  ...
  # Select adaptive M for each k
  for (k in seq_along(ks)) {
    Ms[k] <- local_M_selection(J, ks[k], M_grid)  # Adaptive M for this k
  }

  # Estimating beta coefficients with adaptive M
  for (k in seq_along(ks)) {
    M_k <- Ms[k]  # Use adaptive M for this k
    for (i_p in 1:p)
    {
      betaCoefAll[k, , i_p] <- beta(J, ks[k], nu, Kernel_function, M_k, a = i_p, delta = 0)
      varbetaAll[k, , , i_p] <- variance.estimator.v2(J, ks[k], nu, Kernel_function, M_k, a = i_p, L, delta = 0)
  ...
  list(betaCoefAll, varbetaAll, varbeta, ks, Ms)  # Also return ks and Ms for reference
}
  ...
  for (c in 1:p) {
    for (r in 0:(nu)) {
      if (!(a == c & r == 0)) {
        ...
        # Now loop over replications since each has different ks and Ms
        for (i_rep in 1:R) {
          for (k in 1:2) {
...
Test_tibble|>mutate(n=n,nu=nu,TV_size=TV_size,Kernel=Kernel)
...
# Per-edge detection rate (only for true edges)
edge_names <- colnames(recovered_matrix)
true_edge_idx <- which(true_graph == 1)
null_edge_idx <- which(true_graph == 0)

# Power per true edge
power_per_edge <- colMeans(recovered_matrix)[true_edge_idx]
names(power_per_edge) <- edge_names[true_edge_idx]
print(power_per_edge)
