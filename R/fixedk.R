...
sim.tvVAR_p7_fk = function(burnin, m, TV_size) {
  A = matrix(c(
    0.5,  0,    0,    0,    0,    0,    0,
    0.3,  0.5,  0,    0,    0,    0,    0,
    0,    0.3,  0.5,  0,    0,    0,    0,
  ...
  x1 = x
  st = 0.1 + TV_size * (1 + exp(-0.005 * (c(1:n) - (n / 2))))^(-1)
  for (tt in 2:n) {
    A.t = A
    A.t[1, 1] = st[tt]
...
# Step 1: off-diagonal edge detection, returns k and M for reuse
NonStGM_edges_only_fk = function(x, Kernel = 'Kernel_Triangular', nu = 2, L = 1, alpha = 0.05) {
...
# BY adjustment and edge list construction
edge_recovery_p7_fk = function(Test_tibble, alpha = 0.05) {
...
# Step 2: node TV testing on sub-system, reusing k and M from step 1
test_node_tv_fk = function(x, node, neighbors, k_full, M_list_full,
...
# Combined accuracy from step 1 edges and step 2 TV
twostep_recovery_p7_fk = function(step1_results, TV_results, alpha = 0.05) {
...
run_twostep_p7_fk = function(R = 100, alpha = 0.05, burnin = 200, m = 4096,
                             TV_size = 0.8, nu = 2, L = 1,
                             Kernel = 'Kernel_Triangular', seed = 1,
                             n_cores = 20) {
  # Pre-generate data sequentially for reproducibility
  set.seed(seed)
  x_list = vector("list", R)
  for (iter in 1:R) {
    x_list[[iter]] = sim.tvVAR_p7_fk(burnin, m, TV_size)
  }
  ...
  clusterExport(cl, c("NonStGM_edges_only_fk", "r_to_loc", "x_list"), envir = environment())

  all_step1 = foreach(iter = 1:R, .packages = c("dplyr", "tibble")) %dopar% {
    res = NonStGM_edges_only_fk(x_list[[iter]], Kernel = Kernel, nu = nu, L = L, alpha = alpha)
    tib = res$Test_tibble |> mutate(i = iter)
    list(tibble = tib, k = res$k, M_list = res$M_list)
  }
  stopCluster(cl)
  ...
  step1 = edge_recovery_p7_fk(All_Test_tibble, alpha = alpha)
  rm(All_Test_tibble); gc()

  # Step 2: parallel node TV testing, reusing k and M from step 1
  cl = makeCluster(n_cores)
  registerDoParallel(cl)
  clusterEvalQ(cl, {
    source('extractVariables.R')
  ...
  clusterExport(cl, c("test_node_tv_fk", "r_to_loc", "x_list", "k_list", "M_list_list", "Edge_list"),
                envir = environment())
  ...
  tv_tibble = foreach(iter = 1:R, .combine = rbind, .packages = c("dplyr", "tibble")) %dopar% {
    ...
    for (node in 1:7) {
      ...
      tv_tib = test_node_tv_fk(x, node, neighbors, k_full, M_list_full,
  ...
  final = twostep_recovery_p7_fk(step1, TV_results, alpha = alpha)
  cat("\nFinal Results:\n")
  print(final)
